...
function create_proxy(target, target_port, process_def){
...
function start_proxy(target, target_port, proxy_port, process_def){
    var server = create_proxy(target, target_port, process_def);
...
function stop_proxy_process(proxy_process_def){
    var proxy_name = proxy_process_def.apps[0].name;
    return ppm2.connect()
        .then((_) => ppm2.stop(proxy_name))
        .then((_) => ppm2.delete(proxy_name));
}

function restart_proxy_process(proxy_process_def){
    var proxy_name = proxy_process_def.apps[0].name;
    return ppm2.connect()
        .then((_) => ppm2.stop(proxy_name))
        .then((proc) => ppm2.start(proxy_process_def));
